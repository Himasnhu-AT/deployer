...
export class Config {
  ...
  static getConfig() {
    ...
    config.be_ping_retry_delay =
      config.be_ping_retry_delay ?? config.be_retry_delay ?? 500;
    ...
    config.alert_on_all_be_failure_streak =
      config.alert_on_all_be_failure_streak ?? 3;

    config.enableSelfHealing = config.enableSelfHealing ?? true;
    config._test_only_chances_of_healing_server =
      config._test_only_chances_of_healing_server ?? 0.5;
  ...
  static configAlgoTypeToLbAlgorithm(configAlgoType: IConfig["_lbAlgo"]) {
    switch (configAlgoType) {
      ...
      default:
        throw new Error(
          `[ConfigError] configAlgoTypeToLbAlgorithm: Didn\'t find ${configAlgoType}`,
        );
    }
  }
}
