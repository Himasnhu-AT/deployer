...
// HealthCheck Class to perform Health Check on Backend Servers
export class HealthCheck {
  ...
  // Constructor to initialize HealthCheck Class
  constructor(
  ...
  // Start Health Check on all Backend Servers
  ...
  // Stop Health Check on all Backend Servers
  ...
  public async performHealthCheckOnAllServers(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Completed Health Check at ${new Date().toString()}. Total Backend Servers online: ${this.healthyServers.length}`,
      );
    } catch (error) {
      console.log(
        `Failed to performHealthCheckOnAllServers due to: ${(error as any)?.message}`,
      );
      console.log(error);
    } finally {
  ...
  // Perform Health Check on a Specific Backend Server
  public async performHealthCheck(
    BEServer: IBackendServerDetails,
    ...
    if (!hasAcquiredLock)
      healthCheckMutexRelease = await this.healthCheckMutex.acquire();
    ...
    try {
      ...
      if (healthCheckMutexRelease)
        console.log(
          `performHealthCheck [${BEServer.url}]: health=${BEServer.getStatus()}`,
        );
    } catch (error) {
      console.log(
        `Failed to performHealthCheckOnAllServers due to: ${(error as any)?.message}`,
  ...
  public static async selfHealBEServer(server: IBackendServerDetails) {
    ...
    const randomShouldHeal =
      CONFIG._test_only_chances_of_healing_server === 0
        ? true
    ...
    try {
      ...
      await ExecCMD(
        `cd ${(globalThis as any).__appBaseDir} && npx ts-node be.index.ts ${port}`,
      );

      return true;
    } catch (error) {
      console.log(
        "\t[ERROR] selfHealBEServer - Command failed to run with error: ",
        error,
      );
      return false;
    }
  ...
  private static generateRandomBoolean(
    likelihoodOfTrue: number = CONFIG._test_only_chances_of_healing_server,
  ): boolean {
